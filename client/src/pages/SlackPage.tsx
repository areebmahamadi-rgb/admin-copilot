...
function SlackRow({ item }: { item: TriageItem }) {
  ...
  const handleDraft = () => {
    ...
    draftReply.mutate({ id: item.id, platform: "slack", title: item.title, snippet: item.snippet, sender: item.sender });
    setShowDraft(true);
  };
      ...
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span className="text-xs font-medium text-primary font-body">#{item.title}</span>
            <span className="text-xs text-muted-foreground font-body">{item.sender}</span>
          ...
          <p className="text-sm text-foreground font-body line-clamp-2">{item.snippet}</p>
          <div className="flex items-center gap-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => markRead.mutate({ itemId: item.id, platform: "slack" })} disabled={markRead.isPending}>
...
export default function SlackPage() {
  ...
  const slackItems = (brief.data?.items ?? []).filter((i) => i.platform === "slack");
  const visible = showNoise ? slackItems : slackItems.filter((i) => i.priority !== "noise");
  const noiseCount = slackItems.length - slackItems.filter((i) => i.priority !== "noise").length;
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-headline text-2xl font-semibold text-foreground">Slack Triage</h1>
        </div>
        <div className="flex items-center gap-2">
          {noiseCount > 0 && (
      ...
      {visible.length === 0 ? (
        <p className="text-center text-muted-foreground py-12 font-body">No Slack messages to show. Slack auth may need to be configured.</p>
      ) : (
        <div className="rounded-lg border border-border bg-card">
          {visible.map((item) => <SlackRow key={item.id} item={item} />)}
        </div>
      )}
    </div>
  );
